...
<div id="modal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white p-4 rounded w-96">
        <input id="name" class="border p-2 w-full mb-2" placeholder="Tên thương hiệu">
    ...
    data.forEach(c => {
        ...
        <tr class="border-t hover:bg-gray-50">
            ...
            <td class="p-2 text-center">
                <button onclick="edit(${c.id}, '${c.name}')" class="text-blue-500">Sửa</button> |
                <button onclick="remove(${c.id})" class="text-red-500">Xóa</button>
            </td>
        </tr>`;
        n++;
    });
...
async function remove(id) {
    ...
    await fetch('/api/brands.php?id=' + id, { method: 'DELETE' });
    load();
}
