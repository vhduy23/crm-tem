...
// Convert string to array images
foreach ($products as &$p) {
    $p['images'] = $p['images'] ? explode(',', $p['images']) : [];
}
unset($p);

// The variables $keyword, $category_id, $brand_id, $sort are expected by the view.
// They come from $filters.
...
if (!function_exists('buildFilterUrl')) {
    function buildFilterUrl($overrides = []) {
        $p = [
            ...
            'cat'   => $overrides['cat']   ?? ($_GET['cat'] ?? 0),
            'brand' => $overrides['brand'] ?? ($_GET['brand'] ?? 0),
            'page'  => $overrides['page']  ?? null,
            'sort'  => $overrides['sort']  ?? ($_GET['sort'] ?? 'newest'),
        ];
        $parts = [];
...
// Using $products instead of $data for the view loop, wait, the old view uses `$data`. 
// Let's alias $data to $products so we don't have to rewrite the HTML loop right now.
$data = $products;
?>
